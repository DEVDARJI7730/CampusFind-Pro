<?php
/**
 * CampusFind Pro - Search & Filter Found Items
 */
require_once dirname(__DIR__) . '/config/config.php';
require_once dirname(__DIR__) . '/config/session.php';
require_once dirname(__DIR__) . '/config/database.php';

$page_title = 'Search Found Items';
...
try {
    ...
    if (!empty($query)) {
        $sql .= " AND (f.title LIKE :query OR f.description LIKE :query)";
        $count_sql .= " AND (f.title LIKE :query OR f.description LIKE :query)";
        $params[':query'] = '%' . $query . '%';
    }
    ...
    if (!empty($location)) {
        ...
        $count_sql .= " AND f.location LIKE :location";
        $params[':location'] = '%' . $location . '%';
    }

    if (!empty($date)) {
        $sql .= " AND f.found_date = :date";
        $count_sql .= " AND f.found_date = :date";
        $params[':date'] = $date;
    }
    ...
    error_log("Search Found page query fail: " . $e->getMessage());
    $items = [];
    $total_pages = 1;
    $categories = [];
}
...
<div class="container py-5 flex-grow-1">
    ...
    <div class="text-center max-width-600 mx-auto mb-5" data-aos="fade-up">
        <h1 class="font-heading fw-800 mb-2">Search Found Items</h1>
        <p class="text-secondary">Browse listings reported found on campus. Use filters to narrow search results.</p>
    </div>
            ...
            <div class="glass-panel p-4 sticky-top" style="top: 100px; z-index: 10;">
                ...
                <form action="search.php" method="GET">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="location" class="form-control form-premium-control" placeholder="e.g. Student Center" value="<?php echo sanitize($location); ?>">
                    </div>

                    <!-- Date -->
                    <div class="mb-4">
                        <label class="form-label text-secondary fw-500">Date Found</label>
        ...
        <div class="col-lg-9" data-aos="fade-left">
            <?php if (empty($items)): ?>
                <div class="glass-panel p-5 text-center my-4">
                    ...
                    <h5 class="text-secondary">No matching found reports available.</h5>
                        ...
                        <div class="col-md-6 col-12">
                            <div class="glass-card h-100 position-relative">
                                ...
                                <div class="p-4">
                                    ...
                                    <a href="view.php?id=<?php echo $item['id']; ?>" class="btn btn-premium w-100">View details & claim</a>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
